...
static bool read_cols_on_row(matrix_row_t current_matrix[], uint8_t current_row) {
  ...
  current_matrix[current_row] = ~(expander_read_r(EXPANDER_REG_GPIOB) << 8 | expander_read_r(EXPANDER_REG_GPIOA));
...
uint8_t matrix_scan(void) {
  ...
  if (timer_elapsed32(matrix_last_modified) > MATRIX_POWER_SAVE) {
    ...
    while (true) {
      ...
      if (!~expander_read_r(EXPANDER_REG_GPIOA) || !~expander_read_r(EXPANDER_REG_GPIOB)) {
        continue;
      }
